...
export class UsersRepository {
  ...
  async create(input: UserInput): Promise<UserRecord> {
    const result = await this.pool.query<UserRow>(
      ...
      [input.email.toLowerCase(), input.displayName, input.cognitoSub, input.isActive]
    );
  ...
  async update(id: string, input: UserInput): Promise<UserRecord | null> {
    const result = await this.pool.query<UserRow>(
      `UPDATE control_plane.users
        ...
        WHERE id = $5
       RETURNING id, email, display_name, cognito_sub, is_active, created_at, updated_at`,
      [input.email.toLowerCase(), input.displayName, input.cognitoSub, input.isActive, id]
    );
    return result.rows[0] ? mapUser(result.rows[0]) : null;
  }

  async delete(id: string): Promise<boolean> {
    const result = await this.pool.query(`DELETE FROM control_plane.users WHERE id = $1`, [id]);
